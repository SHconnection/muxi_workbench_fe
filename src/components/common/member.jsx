        ...
        this.props.members.map((item, index) => {
          return (
            <div className = {this.props.wrap ? "unit" : "unit nowrap"} key = {index}>
              <input type = "checkbox" checked = {item.selected} onChange = {this.props.dis ? this.onlyOne.bind(this, item) : this.select.bind(this, item)} id = {"check" + item.name + index}/>
              <label htmlFor = {"check" + item.name + index} className = "fontColor">{item.name}</label>
            </div>
          );
        })
      }

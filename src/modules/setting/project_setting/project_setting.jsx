    ...
    return (
      <div className = "subject">
        ...
        <div className = "select">
          <button className = "certain"><Link to = "/editMem" className = "link">保存</Link></button>
          <button className = "delete" onClick = {() => {this.setState({delete: true})}}>删除项目</button>
          <span className = "cancel">取消</span>
        </div>
        <div className = {this.state.delete ? "subject alert" : "none"}>
          <span>确认要删除该项目吗?</span>
          <div className = "mar">
            <button className = "delete littleBtn" onClick = {() => {this.setState({delete: false})}}>取消</button>
            <button className = "certain littleBtn">确定</button>
          </div>
        </div>
      </div>
    );
